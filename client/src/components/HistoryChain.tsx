    ...
    <section id="history-chain" className="mx-auto max-w-[1440px] border-x border-[#28302e]/10 px-6 py-20 md:px-12 lg:px-20 lg:py-28">
      ...
      <div className="mt-12 grid border-y border-[#28302e]/15 lg:grid-cols-[270px_1fr]">
        ...
        <article className="chain-leaf px-0 py-8 lg:px-10 lg:py-10">
          ...
          {branches.length > 0 && <div className="branch-reading mt-8"><div className="flex items-center gap-3"><Split size={17} className="text-[#4f8c85]" strokeWidth={1.4} /><p className="eyebrow">假如沿另一条线阅读</p></div><div className="mt-5 grid gap-4 lg:grid-cols-2">{branches.map((branch) => { const target = EVENT_NAMES[branch.targetId]; return <button key={branch.id} type="button" onClick={() => openTimelineEvent(branch.targetId)} className="branch-leaf text-left"><p className="font-mono text-[10px] tracking-[.16em] text-[#4f8c85]">{branch.eyebrow}</p><h4 className="mt-3 font-serif text-2xl font-bold tracking-[-.04em]">{branch.title}</h4><p className="mt-3 text-sm leading-7 text-[#71817d]">{branch.description}</p>{target && <span className="mt-5 inline-flex items-center gap-2 text-xs text-[#397b74]">前往 {target.year} · {target.title}<ChevronRight size={13} /></span>}</button>; })}</div><p className="mt-4 text-[11px] leading-5 text-[#71817d]">分支提示提供另一种阅读次序，不是假设历史会以该路径必然发生。</p></div>}
